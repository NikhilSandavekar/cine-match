...
export default function Home() {
  ...
  const sendMagicLink = async () => {
    if (!supabase || !email || emailCooldown) return;
    const { error } = await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: window.location.origin } });
    if (error) { setAccountNote(error.message); return; }
    setAccountNote('Link sent. Please wait 60 seconds before requesting another one.');
    setEmailCooldown(true);
    window.setTimeout(() => setEmailCooldown(false), 60_000);
  };
